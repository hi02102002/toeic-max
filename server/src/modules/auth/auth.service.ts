...
import type { ForgotPasswordDto, IdTokenDto, LoginDto, RegisterDto, RequestVerifyAccountDto, ResetPasswordDto } from './auth.dto';
import { HttpException } from '@/exceptions/http-exception';
import { redis } from '@/libs/redis';
import { logger } from '@/utils/logger';
import { ACCESS_TOKEN_SECRET, REFRESH_TOKEN_SECRET, RESET_PASS_TOKEN_SECRET, VERIFY_EMAIL_TOKEN_SECRET } from '@config';
...
export class AuthService {
  ...
  public async login(fields: LoginDto): Promise<TTokenData> {
    ...
    if (!user) {
      throw new HttpException(StatusCodes.NOT_FOUND, 'Your email is not registered. Please register first.');
    }

    const isMatched = await this.validatePassword(fields.password, user.password);

    if (!isMatched) {
      throw new HttpException(StatusCodes.UNAUTHORIZED, 'Your password is incorrect.');
    }

    if (user.provider !== 'local') {
      throw new HttpException(
        StatusCodes.UNAUTHORIZED,
        'Your account is registered with a different provider. Please login with ' + user.provider + '.',
      );
    }

    if (!user.isVerified) {
      throw new HttpException(StatusCodes.UNAUTHORIZED, 'Your email is not verified. Please verify your email first.', 'EMAIL_NOT_VERIFIED');
    }

    const tokenData = this.genTokens(user);

    await redis.set(`refresh-token:${user.id}`, tokenData.refreshToken, 'EX', TIME_EXPIRED_REFRESH_TOKEN);
  ...
  public async loginWithIdToken(fields: IdTokenDto) {
    ...
    const { email, provider, avatar, name, isVerified } = await this.firebaseService.verifyIdToken(idToken);

    const user = await this.userService.findOneByEmail(email);

    // If user exist and provider is not the same => throw error
    if (user && user.provider !== provider) {
      throw new HttpException(StatusCodes.CONFLICT, 'Your email is registered with a different provider. Please login with ' + user.provider + '.');
    }
    ...
    await redis.set(`refresh-token:${createdUser.id}`, tokenData.refreshToken, 'EX', TIME_EXPIRED_REFRESH_TOKEN);
  ...
  public async loginAdmin(fields: LoginDto): Promise<TTokenData> {
    ...
    if (!user.roles.includes('ADMIN')) {
      throw new HttpException(StatusCodes.UNAUTHORIZED, 'You do not have permission to access this page.');
    }
  ...
  public async register(fields: RegisterDto, clientUrl = 'http://localhost:3000/') {
    const { email, name, password } = fields;
    const user = await this.userService.findOneByEmail(email);

    if (user) {
      throw new HttpException(StatusCodes.CONFLICT, 'Your email is already registered.');
    }
  ...
  public async forgotPassword(fields: ForgotPasswordDto, clientUrl = 'http://localhost:3000/') {
    const { email } = fields;
  ...
  public async resetPassword(
    ...
    const decoded = (await this.verifyToken(token, RESET_PASS_TOKEN_SECRET as string)) as { id: string };
    ...
    if (password !== passwordConfirm) {
      throw new HttpException(StatusCodes.BAD_REQUEST, 'Password and confirm password do not match. Please check your password again.');
    }
  ...
  public async refreshToken(refreshToken: string): Promise<TTokenData> {
    ...
    const decoded = (await this.verifyToken(refreshToken, REFRESH_TOKEN_SECRET as string)) as { id: string };
    ...
    if (blackListToken) {
      ...
      throw new HttpException(StatusCodes.BAD_REQUEST, 'Token is invalid or expired. Please login again to get a new token.');
    }
    ...
    if (!user) {
      ...
      throw new HttpException(StatusCodes.NOT_FOUND, 'Your account is not found. Please login again to get a new token.');
    }
  ...
  private genTokens(user: InferResultType<'users'>): TTokenData {
    ...
    const refreshToken = sign(dataStoredInToken, REFRESH_TOKEN_SECRET as string, {
      expiresIn: '7d',
    });
  ...
  private async validatePassword(incomePassword: string, hashedPassword: string) {
    const isMatched = await bcrypt.compare(incomePassword, hashedPassword);

    return isMatched;
  }

  public async verifyToken(token: string, secret: string, errMsg = 'Token is invalid') {
  ...
  public async requestVerifyAccount(fields: RequestVerifyAccountDto, clientUrl = 'http://localhost:3000/') {
    const { email } = fields;

    const user = await this.userService.findOneByEmail(email);

    if (!user) {
      throw new HttpException(StatusCodes.NOT_FOUND, 'This account is not found.');
    }

    if (user.isVerified) {
      throw new HttpException(StatusCodes.BAD_REQUEST, 'Your email is already verified.');
    }
  ...
  public async verifyAccount(token: string) {
    ...
    if (user.isVerified) {
      throw new HttpException(StatusCodes.BAD_REQUEST, 'Your email is already verified. Please login or reset your password if you forgot it.');
    }
