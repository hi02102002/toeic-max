...
export class FirebaseService {
  async uploadFile(file: Express.Multer.File) {
    ...
    try {
      ...
      logger.info(`[FIREBASE] Delete file on ${file.path} after upload to firebase`);
    } catch (error) {
      logger.error(`[FIREBASE] this file ${file.path} not exist but i will ignore it`);
    }
  ...
  async uploadFiles(files: Express.Multer.File[]) {
    const urls = await Promise.all(files.map(async file => this.uploadFile(file)));
