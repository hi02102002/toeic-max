...
export const callApiDecrypt = async (url: string, method: string, params: Record<string, any>) => {
  const data = await axios({
    ...
    .then(res => {
      ...
      if ('data' in response) {
        const encrypted_json = JSON.parse(atob(response.data.substr(13)));

        const decryptedJson = cryptoJs.AES.decrypt(encrypted_json.value, cryptoJs.enc.Base64.parse(APP_MIX), {
          iv: cryptoJs.enc.Base64.parse(encrypted_json.iv),
        });

        const originalText = JSON.parse(decryptedJson.toString(cryptoJs.enc.Utf8));
...
export const callApi = async (url: string, method: string, params: Record<string, any>, replaceBaseURL = false) => {
