...
import { ForgotPassword, ForgotPasswordProps, VerifyAccount, VerifyAccountProps } from './templates/index';
...
export const sendMail = async <T extends keyof TemplateProps = keyof TemplateProps & string>(opts: {
  to: string;
  subject: string;
  template: T;
  ...
  const { to, subject, template, props, pass = MAIL_PASS, user = MAIL_USER, from = MAIL_USER } = opts;
  try {
