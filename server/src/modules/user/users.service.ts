...
import { BaseService } from '@/libs/api';
import { TNewUser, TUser } from './user.type';

@Service()
export class UserService extends BaseService<TNewUser, Partial<TNewUser>, TUser> {
  ...
  public async changePassword(id: string, fields: ChangePasswordDto) {
    ...
    if (!isPasswordMatch) {
      throw new HttpException(StatusCodes.BAD_REQUEST, 'Old password is incorrect');
    }
