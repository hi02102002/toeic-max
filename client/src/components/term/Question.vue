<template>
    <div class="space-y-2 select-none">
        <span class="font-medium">
            Question {{ question.location }}. {{ question.q?.replace(`${question.location}.`,
                '').replace(question.location.toString(),'') }}
        </span>
        <RadioGroup v-model:model-value="choose">
            <template v-for="(value, key) in question.opts" :key="key + question.id">
                <Button variant="ghost" class="p-1 h-auto justify-start  cursor-pointer">
                    <RadioGroupItem :id="`question-${question.id}-choice-${key}`" :value="key">
                        {{ key }}
                    </RadioGroupItem>
                    <Label :for="`question-${question.id}-choice-${key}`"
                        class="flex-1 text-start cursor-pointer pl-2 block">
                        {{ key.toUpperCase() }}.
